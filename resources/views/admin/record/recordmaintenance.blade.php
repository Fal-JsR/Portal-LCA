...
<div class="container mx-auto px-4 py-8">
    <h1 class="text-3xl font-bold mb-6 text-center text-gray-800">RECORD MAINTENANCE - {{ $instansi->nama_instansi }}</h1>

    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        @forelse($records as $record)
        <div class="bg-white shadow-lg rounded-2xl p-6 mb-4">
            <div class="mb-2">
                <span class="px-3 py-1 rounded-full text-xs font-semibold {{ $record->jenis == 'Kunjungan' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800' }}">
                    {{ $record->jenis }}
                </span>
            </div>
            <div class="mb-2">
    ...
    <div class="text-center mt-8">
        <a href="{{ route('admin.record.index') }}" class="px-6 py-2 bg-gray-500 text-white rounded hover:bg-gray-600 transition-colors">
            <i class="fas fa-arrow-left mr-2"></i>Kembali ke Record Index
        </a>
    </div>
</div>
...
<div id="image-popup" class="fixed inset-0 bg-black bg-opacity-80 z-50 hidden items-center justify-center">
    <div class="bg-white rounded-lg w-full max-w-2xl max-h-screen p-4 relative flex flex-col items-center">
        <button onclick="closeImagePopup()" class="absolute top-2 right-4 text-gray-500 hover:text-gray-700 text-3xl">&times;</button>
        <img id="popup-img" src="" alt="Full Maintenance Image" class="w-full h-auto max-h-[70vh] object-contain rounded">
    </div>
</div>
