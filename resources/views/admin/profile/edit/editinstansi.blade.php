...
@section('content')
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($instansis as $instansi)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $instansi->users->count() }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    {{ $instansi->created_at->format('d M Y') }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <button
                                        onclick="openEditModal({{ $instansi->id }}, '{{ $instansi->nama_instansi }}', '{{ $instansi->created_at->format('Y-m-d') }}')"
    ...
    <div id="editModal" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden items-center justify-center">
        <div class="bg-white rounded-lg w-11/12 max-w-md p-6 relative">
            ...
            <form id="editForm" method="POST">
                ...
                @method('PUT')
                <div class="mb-4">
                    <label for="edit_nama_instansi" class="block text-sm font-medium text-gray-700 mb-2">Nama
                        Instansi</label>
    ...
    <div id="deleteModal" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden items-center justify-center">
        <div class="bg-white rounded-lg w-11/12 max-w-md p-6 relative">
            ...
            <div class="mb-4">
                <p class="text-gray-700">Apakah Anda yakin ingin menghapus instansi <strong
                        id="deleteInstansiName"></strong>?</p>
